function [encodedBuffer] = steganographicEncoding(frameBuffer,M,N,C,F,alpha,sigma)
...
code = mapped*(alpha);
data = imgaussfilt(code,sigma);

for i = 1:size(frameBuffer,4)
    if (rem(i,2) == 0)
        encodedBuffer = (frameBuffer + data)/255;
    else
        encodedBuffer = (frameBuffer + (data)*(-1))/255;
    end
end
end
